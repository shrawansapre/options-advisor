...
- **Unusual Options Scanner** — new `/scanner` route; Worker `chain=full` mode (7–120 DTE, strikeLimit=20, no delta filter); `useOptionsChain` hook with AbortController; `unusualSignals.js` pure functions (findUnusualContracts, callPutRatio, flowSentiment, unusualTilt); Scanner page shell + ScannerInput (collapsible filters) + ScannerSummary (price/IV/C/P ratio) + sortable UnusualTable + expandable ContractRow; `scanner.css` with responsive breakpoints + dark mode; sentiment tokens (`--bullish`, `--bearish`, `--neutral`) in `app.css`; 10 commits to main, deployed to Vercel. Worker deploy still pending (manual step).
...
- **Learn page mobile fix** — pill tabs replaced with underline-scroll nav; card padding tightened; topbar slimmed; comparison table labels shortened for narrow screens

- **Architecture hardening** — 10 fixes across the pipeline: (1) `buildLiveDataBlock` moved from `orchestrator.js` → `utils.jsx`, breaking the critic.js↔orchestrator.js circular import; (2) `closeTab` anti-pattern fixed — `setActiveId` no longer called inside `setAnalyses` updater; (3) critic-skip progress event added so UI can show when critic is bypassed; (4) `ensureTradeShape` fills required trade field defaults after JSON parse, preventing component crashes on malformed strategist output; (5) real batch checklist audit — `checklistAuditorBatch` now issues one Haiku call using `CHECKLIST_AUDITOR_BATCH_PROMPT` (was 3 parallel calls); (6) abort signal threaded into `checklistAuditor` and `checklistAuditorBatch`; (7) worker auth — `X-Internal-Token` header validated against `env.INTERNAL_TOKEN` secret (skipped if secret unset, for dev compatibility); (8) `console.error` observability added to worker catch blocks (wrangler.toml already had logs enabled); (9) KV-aware market cache in worker — uses `env.MARKET_CACHE` KV if bound, falls back to in-memory Map; (10) request deduplication in `App.jsx` — in-flight ticker Set prevents double-submits
