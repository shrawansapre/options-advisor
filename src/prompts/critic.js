...
6. Risk tier (structural AND dollar ordering) — conservative must be defined-risk with high probability of profit (tight credit spread, cash-secured put, or covered call). Moderate must be balanced defined-risk (ATM/near-money long option or moderate spread). Aggressive must be the highest-risk trade (OTM long option, wide spread, or undefined risk). Flag as HIGH severity if ANY of these are true: (a) conservative uses a long OTM option; (b) aggressive uses a tight credit spread that is less risky than moderate or conservative; (c) moderate's max loss exceeds aggressive's max loss — moderate must sit between conservative and aggressive in both structural risk and dollar max loss; (d) the structural riskiness ordering is inverted in any direction.
